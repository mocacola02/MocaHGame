...
// ShortCutListSounds.
//
// List of sounds to track in the shortcut browser.
// 
...
//= Consts =//
const MAX_ROWS= 1500;			// Max rows constant. Never used.

//= Main Vars =//
var int MasterCount;			// Master count value.
var string MasterList[1500];	// Static string array of sound names.
var string status[1500];		// Static string array of sound statuses.
var string MasterText[1500];	// Static string array of descriptions.
...
function Created()
{
	...
	// Add index, status, name, and description columns
...
function PaintColumn (Canvas C, UWindowGridColumn Column, float MouseX, float MouseY)
{
	...
	// If showing horizontal scroll bar, account for it in bottom margin
	if ( bShowHorizSB )
	{
	...
	// If last row exceeds number of rows, set last row to number of rows
	if ( LastRow > NumRows )
	{
		LastRow = NumRows;
	}
	...
	// Max our green value of draw color
	C.DrawColor.G = 255;

	// While current row is less than last row
	while ( CurRow < LastRow )
	{
		// If current row is selected row, zero out red and blue values
		...
		switch (Column.ColumnNum)
		{
			...
			case 1:
				...
				// Exit switch statement
				break;
			// In the case of ColumnNum == 2
			case 2:
				// Clip sound name text
				Column.ClipText(C,2.0,TopMargin + CurOffset,MasterList[CurRow]);
				// Exit switch statement
				break;
			// In the case of ColumnNum == 3
			case 3:
				// Clip sound desc. text
...
// Launches a shortcut from the given row, does nothing without extension
...
function MouseMove (float X, float Y)
{
	...
	if ( bMouseDown )
	{
		...
		// Set selected row to vertical scrollbar position
...
function Reset()
{
	...
	// Set row count to master count
	NumRows = MasterCount;

	// For each row
	for(I = 0; I < NumRows; I++)
	{
		// If current master text is blank, set status to "NOTXT"
		if ( MasterText[I] == "" )
		{
			status[I] = "NOTXT";
		}
		...
		{
			// If there is no sound, set status to "NOSND"
			if ( Sound == None )
			{
				status[I] = "NOSND";
			}
...
// Sort a column, does nothing without extension
...
// Contains a manual mapping of all sounds
defaultproperties
{
