...
function HandlePickup(Actor Other)
{
	if ( (Other.IsA('HPawn') || Other.IsA('HActor')) && ( Weapon.IsA('HWeapon') && HWeapon(Weapon).bAllowPickup))
	{
		ChangeWeapon(class'CarryActor');
		if (Weapon.IsA('CarryActor'))
		{
			CarryActor(Weapon).CarryingActor = Other;
		}
	}
}
...
state statePickupItem
{
	...
	begin:
		...
		if ( CarryActor(Weapon).CarryingActor != None )
		{
			TurnTo(CarryActor(Weapon).CarryingActor.Location * vect(1.00,1.00,0.00) + Location * vect(0.00,0.00,1.00));
		}
...
function StartBossEncounter (baseBoss Boss, bool in_bHarryShouldLockOntoBoss, bool in_bReverseInput, bool in_bKeepHarryFixed, bool in_bCanUseWeapon, Vector in_FixedFaceDirection, class<Spell> ForceSpellType, float in_SpellTargetRange, bool in_bDontShowBossMeter)
{
	...
	{
		baseWand(Weapon).SetForcedSpell(ForceSpellType);
	}

	SpellCursor.CursorRange(in_SpellTargetRange);
...
function StopBossEncounter()
{
	...
	{
		baseWand(Weapon).SetForcedSpell(None);
		baseWand(Weapon).CurrentSpell = None;
	}
...
function HarryKnockBack()
{
	...
	if ( Weapon.IsA('CarryActor') )
	{
		CarryActor(Weapon).DropCarryingActor(false);
	}
...
function TakeDamage (int Damage, Pawn InstigatedBy, Vector HitLocation, Vector Momentum, name DamageType)
{
	...
	local bool bWeaponIsCarry;

	bWeaponIsCarry = Weapon.IsA('CarryActor');
	
	if (bWeaponIsCarry && (CarryActor(Weapon).CarryingActor == InstigatedBy) )
	{
		return;
	}
	...
	FlashScale = FClamp(FinalDamage,20.0,60.0);

	switch (DamageType)
	{
		case 'Falling':
			ClientFlash(-0.02 * FlashScale,FlashScale * vect(20.00,20.00,20.00));
			break;
		case 'Ectoplasma':
			if (  !bEctoFlashed )
			{
				bEctoFlashed = True;
				ClientFlash(-0.03 * FlashScale,FlashScale * vect(9.38,14.06,4.69));
			}
			break;
		case 'PoisonCloud':
			ClientFlash(-0.01171875 * FlashScale,FlashScale * vect(7.80,11.72,11.72));
			break;
		default:
	}
	...
	if ( !HarryIsDead() )
	{
		...
		if ( bWeaponIsCarry )
		{
			CarryActor(Weapon).DropCarryingActor();
		}
...
state Mounting
{
	...
	begin:
		...
		MountDelta -= Normal(MountDelta * vect(1,1,0)) * 30;
...
state statePickBitOfGoyle
{
	begin:
		...
		Acceleration = vect(0,0,0);
		StopAllMusic(0);
		PlayMusic("sm_Ingredient_Success_Music",0);
...
state PlayerWalking
{
	...
	function Landed(vector HitNormal)
	{
			...
			&& (!bNoFallingDamage && NoFallingDamageTimer == 0) ) //only take damage if no falling damage stuff is off (UTPT didn't add this) -AdamJD
		{
			...
			if( fFallDistanceZ > FALL_DAMAGE_DISTANCE )
			{
				...
				else if( fFallDistanceZ < FALL_DAMAGE_DISTANCE + 64 )	// 
				{
					TakeDamage(30, self, location,vec(0,0,0), 'Falling' );
				}
				else if( fFallDistanceZ < FALL_DAMAGE_DISTANCE + 256 )	// 
				{
					TakeDamage(50, self, location,vec(0,0,0), 'Falling' );
				}
				else if( fFallDistanceZ < FALL_DAMAGE_DISTANCE + 512 )	// 
				{
					TakeDamage(100, self, location,vec(0,0,0), 'Falling' );
				}
				else if( fFallDistanceZ < FALL_DAMAGE_DISTANCE + 1024 )	// 
				{
	...
	event PlayerTick( float DeltaTime )
	{
		...
		if( GetHealthCount() <= 0 && !IsInState('stateDead'))
		{
			KillHarry(true);
			return;
		}
		...
		if( bIsAiming && HarryAnimChannel.IsInState('stateCasting') && bAltFire == 0 )
		{
			...
			else if(SpellCursor.IsLockedOn() || bHarryUsingSword && baseWand(weapon).SwordChargedUpEnough() || !baseWand(weapon).bAutoSelectSpell)
			{
	...
	function ProcessMove(float DeltaTime, vector NewAccel, eDodgeDir DodgeMove, rotator DeltaRot)	
	{
		...
		if ( (Physics == PHYS_Walking)  )
		{
			...
			if(   (!bAnimTransition || (AnimFrame > 0)) && !( AnimSequence == HarryAnims[HarryAnimSet].Land && (Speed < 5 || VSize2D(acceleration)==0) ))
			{
				if( Speed > 5 )
				{
				...
				if(  Acceleration != vect(0,0,0) && Speed > 1  )
				{
					bAnimTransition = true;
					TweenToRunning(0.4);
				}
...
function bool CutQuestion(string question)
{
	...
	else if (question ~= "IsGryffindorAhead" || question ~= "IsSlytherinAhead"  || question ~= "IsHufflepuffAhead" || question ~= "IsRavenclawAhead")
	{
...
function UpdateChallengeScores (string strLevelURL, int nHighScore, int nMaxScore)
{
	...
	else if ( InStr(strLevelURL,"CH2SKURGE") != -1 )
	{
		nIdx = nSKURGE_CHALLENGE_IDX;
	}
	else if ( InStr(strLevelURL,"CH3DIFFINDO") != -1 )
	{
		nIdx = nDIFFINDO_CHALLENGE_IDX;
	}
	else if ( InStr(strLevelURL,"CH4SPONGIFY") != -1 )
	{
