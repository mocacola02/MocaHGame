...
//
// Determines how much time we have in the bean bonus room.
// KW coded this like absolute utter garbage. Pirate Software tier coding
// 
...
// Returns the timer duration based on housepoints
function float GetTimerDuration()
{
	local int nGryffPts, nRavenPts, nHuffPts, nSlythPts;
	local int nBestNonGryffPts, nGryffAheadBy;
	local harry PlayerHarry;
	local StatusGroup sgHousePts;

	// If duration is not 0, return the current duration
	if ( fDuration != 0 )
	{
		return fDuration;
	}
	...
	// Get the non-Gryffindor house with the most points
	nBestNonGryffPts = Max(nRavenPts,nHuffPts);
	nBestNonGryffPts = Max(nBestNonGryffPts,nSlythPts);

	// If Gryffindor isn't higher, log and error and use the non-Gryffindor count
	...
	// Get the difference in the two houses' points
	nGryffAheadBy = nGryffPts - nBestNonGryffPts;

	// Hellish condition block. It's not even ordered right. The 185 check is out of order and will never pass.
	// Anyway, determine duration based on how far ahead Gryffindor is
